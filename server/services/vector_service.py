...
from services.embeddings import DashScopeEmbeddings
...
class VectorService:
    ...
    def __init__(self):
        ...
        self.embeddings = DashScopeEmbeddings(
            api_key=current_app.config['EMBED_API_KEY'],
            base_url=current_app.config['EMBED_BASE_URL'],
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                current_app.logger.warning(
                    f'嵌入请求失败(第{attempt + 1}次)，{wait}秒后重试: {err_msg}'
                )
                time.sleep(wait)
        raise last_error
